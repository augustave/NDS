...
- **[`joe-gebbia-nds-chief-design-officer`](../SKILL.md)** — Civic CDO under EO 14338. Often the *upstream* of a Spatial-Motion engagement: when a CDO assessment recommends a service redesign and stakeholders need to *see* it before approving. Handoff artifact received: `executive_synthesis.md` (CDO) → `technical_system_data` + `spatial_interaction_spec` (this skill).

- **[`autonomous-venture-studio`](../autonomous-venture-studio/SKILL.md)** — 0-to-1 founder skill. Often the *upstream* of a Spatial-Motion engagement when a venture concept needs an immersive demo for investor or executive review. Handoff artifact received: `executive_launch_decision_memo.md` + `product_architecture` (AVS) → `technical_system_data` + `spatial_interaction_spec` (this skill).
